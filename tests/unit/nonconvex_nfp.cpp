...
auto count_vertex_kind(const shiny::nesting::ArrangementGraph &graph,
                       shiny::nesting::GraphVertexKind kind) -> std::size_t {
  return static_cast<std::size_t>(
      std::count_if(graph.vertices.begin(), graph.vertices.end(),
                    [kind](const shiny::nesting::GraphVertex &vertex) {
...
void require_valid_vertex_references(
    const shiny::nesting::ArrangementGraph &graph) {
...
TEST_CASE("arrangement graph inserts intersection and midpoint vertices",
          "[nfp][nonconvex][graph]") {
  const shiny::nesting::NonconvexNfpRequest request{
      .piece_a_id = 201,
      .piece_b_id = 202,
  ...
  const shiny::nesting::decomp::DecompositionResult piece_a{
      .components =
          {
      ...
      .validity = shiny::nesting::decomp::DecompositionValidity::valid,
      .signed_area = 5.0,
  };
  const shiny::nesting::decomp::DecompositionResult piece_b = piece_a;

  const auto graph =
      shiny::nesting::detail::build_arrangement_graph_from_decompositions(
      ...
      .vertices =
          {
              {.id = 0,
               .point = {0.0, 0.0},
               .kind = shiny::nesting::GraphVertexKind::original_vertex},
              {.id = 1,
               .point = {2.0, 0.0},
               .kind = shiny::nesting::GraphVertexKind::midpoint_vertex},
              {.id = 2,
               .point = {0.0, 0.0},
               .kind = shiny::nesting::GraphVertexKind::original_vertex},
              {.id = 3,
      ...
      .edges =
          {
              {.from = 0,
               .to = 1,
               .kind = shiny::nesting::GraphEdgeKind::boundary_edge},
              {.from = 2,
               .to = 3,
               .kind = shiny::nesting::GraphEdgeKind::boundary_edge},
          },
      ...
      .vertices =
          {
              ...
              {.id = 4,
               .point = {2.0, 0.0},
               .kind = shiny::nesting::GraphVertexKind::intersection_vertex},
              {.id = 5,
               .point = {0.0, 0.0},
               .kind = shiny::nesting::GraphVertexKind::intersection_vertex},
          },
      ...
      .edges =
          {
              ...
              {.from = 1,
               .to = 0,
               .kind = shiny::nesting::GraphEdgeKind::pruned_edge},
          },
      .pruned = true,
  };
...
TEST_CASE("extract_nfp_from_graph classifies nested loops as holes",
  ...
  REQUIRE(result.loops[0].kind == shiny::nesting::NfpFeatureKind::outer_loop);
  REQUIRE(result.loops[1].kind == shiny::nesting::NfpFeatureKind::hole);
}
...
TEST_CASE("nonconvex nfp engine reuses cached decomposition inputs",
  ...
  shiny::nesting::NfpEngine engine{};
  ...
  const shiny::nesting::NonconvexNfpRequest request{
      ...
      .algorithm_revision = shiny::nesting::cache::AlgorithmRevision{9},
  };

  const auto first = engine.compute_nonconvex_graph_nfp(
      request, shiny::nesting::cache::GeometryRevision{5},
      shiny::nesting::cache::GeometryRevision{7});
  ...
  REQUIRE(first.status == shiny::nesting::ExtractionStatus::success);
...
TEST_CASE("nonconvex cache key changes with algorithm revision",
  ...
  shiny::nesting::NonconvexNfpRequest request{
      .piece_a_id = 301,
      .piece_b_id = 302,
      .piece_a = {.outer = {{0.0, 0.0},
                            {4.0, 0.0},
  ...
  const auto first = engine.compute_nonconvex_graph_nfp(
      request, shiny::nesting::cache::GeometryRevision{2},
      shiny::nesting::cache::GeometryRevision{3});
  REQUIRE(engine.nonconvex_cache_size() == 1U);
  REQUIRE(first.status == shiny::nesting::ExtractionStatus::success);

  request.algorithm_revision = shiny::nesting::cache::AlgorithmRevision{2};
  const auto revised = engine.compute_nonconvex_graph_nfp(
      request, shiny::nesting::cache::GeometryRevision{2},
      shiny::nesting::cache::GeometryRevision{3});
  REQUIRE(engine.nonconvex_cache_size() == 2U);
  REQUIRE(revised.status == shiny::nesting::ExtractionStatus::success);
}
...
TEST_CASE("nonconvex engine does not cache interrupted requests",
  ...
  const shiny::nesting::NonconvexNfpRequest request{
      ...
      .rotation_a = {.degrees = 0.0},
      .rotation_b = {.degrees = 0.0},
      .algorithm_revision = shiny::nesting::cache::AlgorithmRevision{3},
  };

  const auto interrupted = engine.compute_nonconvex_graph_nfp(
      request, shiny::nesting::cache::GeometryRevision{8},
      shiny::nesting::cache::GeometryRevision{9}, []() { return true; });
