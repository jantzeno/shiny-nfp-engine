...
auto polygon_signed_area(const shiny::nfp::geom::Ring &ring) -> long double {
...
TEST_CASE("svg path polygonization covers line and curve commands",
          "[io][svg]") {
  const auto root = shiny::nfp::test::load_fixture_file("io/svg_paths.json");
  ...
  for (const auto &fixture_node : root.get_child("fixtures")) {
    ...
    DYNAMIC_SECTION(id) {
      const auto result = shiny::nfp::io::polygonize_svg_path(
      ...
      if (const auto expected_polygon =
        ...
        shiny::nfp::test::require_polygon_equal(
            result.value(), shiny::nfp::test::parse_polygon(*expected_polygon));
      }
    }
  }
}
...
TEST_CASE("svg path polygonization rejects invalid open paths", "[io][svg]") {
  const auto result =
      shiny::nfp::io::polygonize_svg_path("M 0 0 L 4 0 L 4 2", {});
  REQUIRE(result.status() == shiny::nfp::util::Status::invalid_input);
}