...
TEST_CASE("orbital verifier engine reuses cached decomposition inputs",
  ...
  REQUIRE(first.result.algorithm ==
          shiny::nesting::AlgorithmKind::orbital_verifier);
...
auto segment_less(const shiny::nesting::geom::Segment2 &lhs,
                  const shiny::nesting::geom::Segment2 &rhs) -> bool {
...
void require_segments_equal(
  ...
  auto normalized = [](std::vector<shiny::nesting::geom::Segment2> segments) {
...
void require_loops_equal(const std::vector<NfpLoop> &actual,
                         ...
                         NfpFeatureKind kind) {
  std::vector<shiny::nesting::geom::Ring> actual_rings;
  std::vector<shiny::nesting::geom::Ring> expected_rings;
...
auto find_graph_fixture(const shiny::nesting::test::pt::ptree &root,
...
auto build_request(const shiny::nesting::test::pt::ptree &inputs)
    -> NonconvexNfpRequest {
  return {
  ...
  for (const auto &fixture_node : orbital_root.get_child("fixtures")) {
    ...
    DYNAMIC_SECTION(id) {
      ...
      const auto graph_result =
          shiny::nesting::compute_nonconvex_graph_nfp(request);
      const auto orbital_result =
          shiny::nesting::compute_orbital_verifier_nfp(request);

      REQUIRE(graph_result.status == shiny::nesting::ExtractionStatus::success);
      ...
      std::vector<shiny::nesting::geom::Point2> expected_inner_loop_starts;
