...
TEST_CASE("strip optimizer improves a constructive seed on a strip-style case",
  ...
  // The bin is exactly filled (100% utilisation), so no reordering can
  // shorten the strip.  The optimizer correctly makes zero accepted moves
  // and preserves all 3 placed pieces from the seed.
