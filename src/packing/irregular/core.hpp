...
[[nodiscard]] inline auto almost_equal(const double lhs, const double rhs) -> bool {
  return std::fabs(lhs - rhs) <= kDistanceEpsilon;
}

[[nodiscard]] inline auto effective_geometry_revision(const std::uint64_t revision,
                                                      const bool mirrored)
    -> std::uint64_t {
...
[[nodiscard]] inline auto allowed_rotations_for(
...
auto apply_candidate(WorkingBin &bin, const CandidatePlacement &candidate,
                     std::vector<PlacementTraceEntry> &trace, bool opened_new_bin,
                     const ExecutionPolicy &execution,
                     const TrialStateRecorder *trial_recorder = nullptr) -> void;
auto remove_piece_from_bins(std::vector<WorkingBin> &bins, std::uint32_t piece_id,
                            const ExecutionPolicy &execution,
                            const TrialStateRecorder *trial_recorder = nullptr) -> bool;
[[nodiscard]] auto collect_recent_piece_ids(
    const std::vector<PlacementTraceEntry> &trace, std::size_t max_count)
...
[[nodiscard]] auto find_best_for_bin(
    ...
    std::size_t total_parts, std::uint64_t per_piece_budget_ms,
...
[[nodiscard]] auto try_place_piece(
    ...
    std::span<const BinInstance> bin_instances, std::vector<WorkingBin> &opened_bins,
    std::vector<bool> &opened_flags, std::vector<PlacementTraceEntry> &trace,
    ...
    std::uint64_t per_piece_budget_ms, cache::NfpCache *nfp_cache,
...
[[nodiscard]] auto try_backtrack_place_piece(
    ...
    std::uint32_t max_backtrack_pieces) -> bool;
...
[[nodiscard]] auto make_budget(const SolveControl &control,
                               ...
                               std::size_t iterations_completed) -> BudgetState;
...
auto emit_search_progress(const SolveControl &control, std::size_t placed_parts,
                          std::size_t total_parts, const BudgetState &budget,
