...
auto better_candidate(const WorkingBin &bin, const place::PlacementPolicy policy,
  ...
  const auto lhs_secondary =
      secondary_edge_distance(container_bounds, lhs.bounds, bin.state.start_corner);
  const auto rhs_secondary =
      secondary_edge_distance(container_bounds, rhs.bounds, bin.state.start_corner);
...
auto fits_bin_direct(const geom::PolygonWithHoles &piece,
  ...
  if (geom::polygon_area_sum(poly::difference_polygons(piece, bin.state.container)) >
      kAreaEpsilon) {
    return false;
  }
...
auto hole_index_for_candidate(const geom::PolygonWithHoles &piece,
                              const geom::Box2 &piece_bbox,
                              const std::span<const geom::PolygonWithHoles> holes)
    -> std::int32_t {
...
auto respects_spacing(const geom::PolygonWithHoles &piece, const WorkingBin &bin,
                      const ExecutionPolicy &execution) -> bool {
