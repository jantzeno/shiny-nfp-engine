...
[[nodiscard]] auto same_decision_identity(const search::SolutionPoolEntry &lhs,
                                          const search::SolutionPoolEntry &rhs)
    -> bool {
  return lhs.order == rhs.order && lhs.piece_indexed_forced_rotations ==
                                       rhs.piece_indexed_forced_rotations;
}
...
auto select_rollback_candidate(
    std::span<const search::SolutionPoolEntry> entries,
    const search::SolutionPoolEntry &incumbent, const std::uint64_t seed,
    runtime::TraceCapture *trace) -> std::optional<RankedRollbackCandidate> {
  for (std::size_t index = 0; index < entries.size(); ++index) {
    if (same_decision_identity(entries[index], incumbent)) {
      continue;
    }
    if (trace != nullptr) {
      trace->infeasible_pool_selections.push_back({
          .seed = seed,
          .piece_id = entries[index].result.layout.placement_trace.empty()
                          ? 0U
                          : entries[index]
                                .result.layout.placement_trace.front()
                                .piece_id,
          .pool_size = entries.size(),
          .rank = index + 1U,
      });
    }
    return RankedRollbackCandidate{
        .entry = entries[index],
        .rank = index + 1U,
    };
  }
...
auto resolve_rollback_candidate(const search::SolutionPoolEntry &incumbent,
  ...
  if (search::detail::primary_metrics_preserved(rollback->entry.metrics,
                                                incumbent.metrics)) {
    return {
        .next_incumbent = rollback->entry,
        .rolled_back = true,
        .rank = rollback->rank,
    };
  }
  return {
      .next_incumbent = incumbent,
      .restored_previous_incumbent = true,
      .rank = rollback->rank,
  };
}
