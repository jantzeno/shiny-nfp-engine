...
[[nodiscard]] auto maximize_value(const LayoutMetrics &metrics) -> bool {
  return metrics.objective_mode == ObjectiveMode::maximize_value;
}

[[nodiscard]] auto placed_value_for_layout(const NormalizedRequest &request,
                                           const pack::Layout &layout)
    -> double {
  std::unordered_map<std::uint32_t, double> value_by_source_piece_id;
  value_by_source_piece_id.reserve(request.request.pieces.size());
  for (const auto &piece : request.request.pieces) {
    value_by_source_piece_id.emplace(piece.piece_id, piece.value);
  }

  std::unordered_map<std::uint32_t, double> value_by_expanded_piece_id;
  value_by_expanded_piece_id.reserve(request.expanded_pieces.size());
  for (const auto &expanded_piece : request.expanded_pieces) {
    const auto value_it =
        value_by_source_piece_id.find(expanded_piece.source_piece_id);
    value_by_expanded_piece_id.emplace(
        expanded_piece.expanded_piece_id,
        value_it == value_by_source_piece_id.end() ? 0.0 : value_it->second);
  }

  double total_value = 0.0;
  for (const auto &placement : layout.placement_trace) {
    const auto value_it = value_by_expanded_piece_id.find(placement.piece_id);
    if (value_it != value_by_expanded_piece_id.end()) {
      total_value += value_it->second;
    }
  }
  return total_value;
}
...
auto metrics_for_layout(const pack::Layout &layout) -> LayoutMetrics {
  ...
  metrics.placed_value = static_cast<double>(metrics.placed_parts);
...
auto metrics_for_layout(const NormalizedRequest &request,
                        const pack::Layout &layout) -> LayoutMetrics {
  auto metrics = metrics_for_layout(layout);
  metrics.objective_mode = request.request.execution.objective_mode;
  if (metrics.objective_mode == ObjectiveMode::maximize_value) {
    metrics.placed_value = placed_value_for_layout(request, layout);
  }
  return metrics;
}
...
auto better_metrics(const LayoutMetrics &lhs, const LayoutMetrics &rhs)
  ...
  if (maximize_value(lhs) || maximize_value(rhs)) {
    if (std::abs(lhs.placed_value - rhs.placed_value) > kValueTolerance) {
      return lhs.placed_value > rhs.placed_value;
    }
    if (lhs.placed_parts != rhs.placed_parts) {
      return lhs.placed_parts > rhs.placed_parts;
    }
  } else if (lhs.placed_parts != rhs.placed_parts) {
    return lhs.placed_parts > rhs.placed_parts;
  }
