...
auto OrderEvaluator::evaluate(
  ...
  if (result_or.ok() &&
    ...
    evaluated.metrics = metrics_for_layout(request_, evaluated.result.layout);
    return evaluated;
  }
...
auto primary_metrics_preserved(const LayoutMetrics &candidate,
  ...
  if (maximize_value(candidate) || maximize_value(reference)) {
    if (std::abs(candidate.placed_value - reference.placed_value) >
        kValueTolerance) {
      return candidate.placed_value > reference.placed_value;
    }
    if (candidate.placed_parts != reference.placed_parts) {
      return candidate.placed_parts > reference.placed_parts;
    }
  } else if (candidate.placed_parts != reference.placed_parts) {
    return candidate.placed_parts > reference.placed_parts;
  }
